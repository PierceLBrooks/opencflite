...
#else
static void __internalStorageCheckIntegrity(CFStorageRef storage, CFStorageNode* node) {
	__CFStorageCheckNodeIntegrity(storage, node);
   if (node->isLeaf)
      return;

	CFStorageNode* childNode = 0;
	if ((childNode = node->info.notLeaf.child[0])) __internalStorageCheckIntegrity(storage, childNode);
	if ((childNode = node->info.notLeaf.child[1])) __internalStorageCheckIntegrity(storage, childNode);
	if ((childNode = node->info.notLeaf.child[2])) __internalStorageCheckIntegrity(storage, childNode);
}

static void __CFStorageCheckIntegrity(CFStorageRef storage) {
   CFStorageNode* node = &storage->rootNode;

   __internalStorageCheckIntegrity(storage, node);
}
#endif
